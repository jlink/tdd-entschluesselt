...
class EncryptingStringWithColumnarTransposition {
    ...
    @Test
    void transpositionKeyCanHandleListsShorterThanKey() {
        def key = new TranspositionKey(0: 1, 1: 5, 2: 4, 3: 3, 4: 0, 5: 2)
        assert key.transpose([0, 1, 2, 3]) == [0, 3, 2, 1]
    }
    ...
    //@Test
    void longerTextWithLongerKey() {
        def key = new TranspositionKey(0: 4, 1: 1, 2: 2, 3: 3, 4: 0, 5: 5)
        ColumnarTranspositionEncryptor encryptor =
                new ColumnarTranspositionEncryptor(key)
        assert encryptor.encrypt("dieschlachtbeginntbeimorgengrauen") ==
                "ctnoriageeeeciinnshnmgdlebguhbtra"
    }

    @Test
    void multirowTranspositionWithNonFittingClearText() {
        def key = new TranspositionKey(0: 1, 1: 2, 2: 0)
        ColumnarTranspositionEncryptor encryptor =
                new ColumnarTranspositionEncryptor(key)
        assert encryptor.encrypt("abcd") == "cadb"
    }
}
